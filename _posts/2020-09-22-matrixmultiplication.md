...
<p>Convolution is the most widely used and most important layer in deep learning neural networks for image classification or regression tasks. Its counterpart, transpose convolution or typically named as ConvTranspose, is also widely used in networks (e.g. UNet) to convert data back to the original image size so that they can be added to or concatenated with the original data to form skip layers. Due to the complexity involved in the forward and backward computation, both convolution and ConvTranspose do not seem as straightforward as other modules. Therefore, we plan to use the following 4 posts to explain the fundamentals of convolution and ConvTranspose with examples of custom implementations and hope to help understand these concepts. </p>

<ul>
	<li><a href="#_The_simple_network">Matrix multiplication and its custom implementation</a></li>
	<li><a href="#_Derivation_of_the_gradients">Conv2d and its custom implementation</a></li>
	<li><a href="#_Custom_implementations_and_validation">ConvTranpose2d and its custom implementation </a></li>
	<li><a href="#_Summary">Application of Conv2d and ConvTranpose2d in Neural Networks</a></li>
</ul>


<p>We take the 1st part to talk about matrix multiplication because it can be used in Convolution and ConvTranspose operations to make things more simple. In this post, we will focus on derivation of the gradients of matrix multiplication. While the derivation process might seem complex, the final results will be in a pretty simple form and easy to remember. This post is organized in the following sections:</p>

<ul>
	<li><a href="#_The_simple_network">Matrix multiplication</a></li>
	<li><a href="#_Derivation_of_the_gradients">2.	Derivation of gradients using backpropagation chain rule</a></li>
	<li><a href="#_Custom_implementations_and_validation">Custom implementations and validation </a></li>
	<li><a href="#_Summary">Summary</a></li>
	<li><a href="#_References">References</a></li>
</ul>

<h3><a name="_The_simple_network"></a>1. 1.	Matrix multiplication</h3>  
<p>Definition of matrix multiplication can be found in every linear algebra book. Let’s use the definition from Wikipedia[1]. Given a \(m \times k\) matrix \(\boldsymbol A\) and a \(k \times n\) matrix \(\boldsymbol B\)</p>
...
<p>If we are considering an isolated matrix multiplication, the partial derivative \boldsymbol C with respect to either \boldsymbol A and \boldsymbol B would be a 4-D hyper-space relationship, referred to as Jacobian Matrix. You will also find that there will be a lot of zeros in the 4-D Jacobian Matrix because, as shown in Equation (3), \({c_{ij} }\) is a function of only the \(i\)th row of \(\boldsymbol A\) and the \(j\)th column of \(\boldsymbol B\), and independent of other rows of \(\boldsymbol A\) and other columns of \(\boldsymbol B\). </p>

<p>What we are considering here is not an isolated matrix multiplication. Instead, we are talking about matrix multiplication inside a neural network that will have a scalar loss function. For example, consider a simple case that the loss \(L\) is the mean of matrix \(\boldsymbol C\):</p>
...
<p>We will use the chain rule to do backpropagation of gradients. For such an important thing in neural networks, it doesn’t hurt to briefly summarize the chain rule just like the previous post for one more time. Given a function \(L\left( { {x_1},{x_2}, \ldots {x_N} } \right)\) as</p>
...
<p>In this section, we will use a \(2 \times 4\) matrix \(\boldsymbol A\) and a \(4 \times 3\) matrix \(\boldsymbol B\) as an example to step-by-step derive the partial derivative of \(\frac{ {\partial L} }{ {\partial \boldsymbol A} }\). Please note that the same derivation can be performed on a general \(m \times k\) matrix \(\boldsymbol A\) and \(k \times n\) matrix \(\boldsymbol B\). \boldsymbol A specific example is used here purely for the purpose to make it more straightforward. </p>
...
<p> Consider an arbitrary element of \(\boldsymbol A\), for example \({a_{23} }\), we have the local partial derivative of \(\boldsymbol C\)  w.r.t. \({a_{23} }\) based on Equation (7). </p>
...
<p> Following the same way, we can derive the other elements of \(\frac{ {\partial L} }{ {\partial \boldsymbol A} }\) as below </p>
...
<p> Following the same way, we can derive the other elements of \(\frac{ {\partial L} }{ {\partial \boldsymbol B} }\) as below </p>
...
<p>With the derived Equations (15) and (20), it is in fact pretty easy to implement the backward pass of matrix multiplication. Please see an example implementation here for a network that simply takes the mean of the matrix product \(\boldsymbol C = \boldsymbol A \boldsymbol B\) as the loss. The core part is just a 3-line code as below.  </p>

<pre class="pre-scrollable">
	<code class="python">
		
        grad_C_manual = (torch.ones(\boldsymbol C.shape, dtype=torch.float64)/\boldsymbol C.numel())
        grad_A_manual = grad_C_manual.mm(\boldsymbol B.t())
        grad_B_manual = \boldsymbol A.t().mm(grad_C_manual)
...
<p>The second and third lines compute \(\frac{ {\partial L} }{ {\partial \boldsymbol A} }\) and \(\frac{ {\partial L} }{ {\partial \boldsymbol B} }\) using the chain rule based on Equations (15) and (20), respectively. The function t() is a matrix transpose operation. </p>

<p>To validate our derivations and implementation, these results are compared to those from Torch built-in implementation via loss.backward() and they match.  </p>


<h3><a name="_Summary"></a>4. Summary </h3>
<p>In this post, demonstrated how to derive the gradients of matrix multiplication in neural networks. While the derivation steps seems complex, the final equations of the gradients are pretty simple and easy to implement:  </p>
...
<p> With matrix multiplication being covered, we will move to Convolution in the next post. Please stay tuned. </p>
