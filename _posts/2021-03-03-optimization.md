...
<ul>
	...
	<li>If the gradient \({g_m}\) is positive, it means that an increase in \({w_n}\) will lead to an increase in loss \(L\). In this case, the adjustment amount \( - \alpha  \cdot {g_m}\) will be negative, so the new \({w_m}\) will be smaller, which in turn will cause the loss to be smaller. Vice versa.</li>
